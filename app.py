...
if "id_token" not in st.session_state:
            ...
            if not email or not password:
                ...
                if "idToken" in res:
                    if check_email_verified(res["idToken"]):
                        ...
                        st.experimental_rerun()  # ONLY here, right after successful login
                    else:
        ...
        if st.button("Sign Up"):
            if not email or not password:
                ...
                if "idToken" in res:
                    ...
                    # No rerun here — wait for email verification
                else:
    ...
    if st.sidebar.button("Logout"):
        ...
        st.experimental_rerun()  # ONLY here on logout
    ...
    if menu == "Home":
            ...
            with st.expander(post["title"]):
                ...
                if st.session_state["user_uid"] not in post["team"]:
                    if st.button("Join Team", key=f"join_{post_id}"):
                        ...
                        st.experimental_rerun()  # ONLY here on join
        ...
        if st.button("Post Idea"):
            if title and description:
                ...
                st.experimental_rerun()  # ONLY here on post
            else:
